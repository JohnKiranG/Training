...
public class DoctorDAOImpl {
	public static void insert(String name, String psswd) {
		...
		try {
			...
			st.executeUpdate("insert into doctor(name,psswd) values ('" + name + "'," + psswd + ")");
	...
	public static void delete(String name, String psswd) {
		...
		try {
			ps = conn.prepareStatement("delete from doctor where name=? and psswd=?");
			ps.setString(1, name);
	...
	public static void update(String name, String newName, String psswd) {
		...
		PreparedStatement ps = null;
		try {
			ps = conn.prepareStatement("update doctor set name=?,psswd=?  where name=? ");
